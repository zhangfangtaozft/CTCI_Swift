...
public extension Collection {
    ...
    func successiveElements(where predicate: (_ previousElement: Iterator.Element, _ element: Iterator.Element) -> Bool) -> IndexElementPair? {
        for (left, right) in indexElementPairs() {
            guard predicate(left.1, right.1) else {
                continue
            }
...
public extension Collection {
    func unsortedRange(isUnorded: (_ left: Iterator.Element, _ right: Iterator.Element) -> Bool) -> ClosedRange<Index>? {
        guard let unsortedStart = successiveElements(where: isUnorded) else {
            return nil
        }
